...
# Executive summary
# -------------------------------------
# Layer 3 expands L2-derived patterns with context-aware co-activation:
# • As in L2, scaling mode (avg vs. avg/sqrt(std)) and feed_list pre-bias are already applied in L1;
#   L3 inherits these pre-scaled, pre-weighted inputs without separate flags.
# • Preserve pinned indices (patterns) from L2 (solution_space) and protect them in search/repair.
# • As in L2, build an L3 working window via global or relationship-aware ordering,
#   anchored on L2’s patterns and context expansion.
# • As in L2, balance samples (ROS -> SMOTE) and re-order,
#   applied to the expanded L3 window.
# • As in L2, run leader+arms adaptive search to expand cardinality 
#   under capacity (k_L2 -> k_L3).
# • As in L2, map results back to original indexing in two steps: augmented -> L3 working view -> original order.
#
# Key flags
# -------------------------------------
# • relationship_control_l3     : True -> relationship-aware (co-activation) ordering;      False -> global ordering.
# • random_control_l3           : True -> protect pinned indices from random flips;         False -> freer edits.
# • level_of_dim_reduction_l3   : Final-stage reduction dimension after data augmentation.
#
# Inputs/Outputs
# -------------------------------------
# In :
# • solution_space (L2-derived pinned, mapped pattern)
# • profits_original (already pre-weighted by feed_list and/or avg vs. avg/sqrt(std) in L1)
# • weights_original
# • my_capacity_layer3 (target pattern size)
# • my_dimension_layer3 (phase-1 reduction dimension)
# • level_of_dim_reduction_l3 (phase-2 reduction dimension)
# • text_dimension (original full dimension)
# • zero_one_matrix
# • random_control_l3
# • relationship_control_l3
#
# Out:
...
# • solution_space_of_selected_items                -> conditional co-activation around L2 pins
...
# • augmented_data / augmented_data_no_constant     -> ROS/SMOTE balancing + re-order of expanded L3 window
# • objective_function                              -> adaptive leader/arms search under capacity (k_L2 -> k_L3)
# • profit_function                                 -> scoring (hit-weighted co-activation across pins)
# • cost_function                                   -> capacity cost (cardinality; unit weights)
# • rearrange_solutions_for_decoding                -> reverse index mapping (augmented -> L3 view -> original order)
...
class Layered_Multiple_Frequency_Optimization_Layer3:
    def __init__(self, solution_space, profits_original, weights_original, 
                 my_capacity_layer3, my_dimension_layer3, text_dimension, level_of_dim_reduction_l3, zero_one_matrix,
                 random_control_l3, relationship_control_l3):

        # -------------------------------------------------------------------------
        # L3 (Solution Space) setup: core config & flags
        # -------------------------------------------------------------------------
        ...
        # Flags & parameters
        self.random_control = random_control_l3
        self.solution_space = solution_space
        self.dimension = my_dimension_layer3
        self.capacity = my_capacity_layer3
        self.text_dimension = text_dimension    
        self.level_of_dim_reduction_l3 = level_of_dim_reduction_l3
        self.zero_one_matrix = zero_one_matrix
        self.relationship_control = relationship_control_l3 
        ...
        if self.relationship_control == False:
            ...
            solution_space, profits, weights, sorting_index_list_l3 = self.sort_profits_and_solutions(self.dimension, self.solution_space, sort_list, converted_profits, self.weights)
        else:
            # Contextual ordering: project around pinned L2 indices
            solution_space, profits, weights, sorting_index_list_l3 = self.solution_space_of_selected_items(self.dimension, self.solution_space, self.profits, self.weights)
        ...
        self.sorting_index_list_l3 = copy.deepcopy(sorting_index_list_l3)
        

        # -------------------------------------------------------------------------
        # Phase-1 dimension reduction: initial working window for Layer 3
        # Limit the working arrays to the top-d items selected/ordered above.
        # -------------------------------------------------------------------------
        
        self.dimension = copy.deepcopy(my_dimension_layer3)
        ...
        self.sorting_index_list_l3 = self.sorting_index_list_l3[:d]
        ...
        augmented_solution_space, augmented_profits, augmented_weights, sorting_augmented_index_list_l3 = self.augmented_data()
        self.solution_space = augmented_solution_space
        self.profits = augmented_profits
        self.weights = augmented_weights
        self.sorting_augmented_index_list_l3 = sorting_augmented_index_list_l3


        # -------------------------------------------------------------------------
        # Phase-2 dimension reduction: final L3 window after augmentation
        # Align all working arrays (and mask, if any) to the final size d.
        # -------------------------------------------------------------------------
        
        self.dimension = copy.deepcopy(self.level_of_dim_reduction_l3)
        d = self.dimension
        self.profits = self.profits[:d]
        self.weights = self.weights[:d]
        self.sorting_augmented_index_list_l3 = self.sorting_augmented_index_list_l3[:d] 
        self.solution_space = self.solution_space[:d]
    ...
    def objective_function(self, s = 1, p = 1, k = 3, m = 1, x = 0):
        d = self.dimension
        ...
        sorting_augmented_index_list_l3 = copy.deepcopy(self.sorting_augmented_index_list_l3)
        final_solution_output = self.rearrange_solutions_for_decoding(final_solution_output, sorting_augmented_index_list_l3, self.dimension_first_phase)
        final_solution = self.rearrange_solutions_for_decoding(final_solution, sorting_augmented_index_list_l3, self.dimension_first_phase)
        
        sorting_index_list_l3 = copy.deepcopy(self.sorting_index_list_l3)
        final_solution_output = self.rearrange_solutions_for_decoding(final_solution_output, sorting_index_list_l3, text_dimension)
        final_solution = self.rearrange_solutions_for_decoding(final_solution, sorting_index_list_l3, text_dimension)
    ...
    def rearrange_solutions_for_decoding (self, final_solution, sorting_index_list, d):
        #d = self.my_dimension_solution_space
    ...
    # - Build mask from L2-derived pattern in final_solution.
    ...
    def solution_space_of_selected_items(self, dim_next, final_solution, profit_matrix, weight_vector):
        ...
        for i in range(d):
            ...
            avg_list.append(my_avg) #for sorting
            my_val = 0
    ...
    # - If relationship_control == False (plain), boost L2-derived patterns (set score=1) so they stay on top.
    ...
    def sort_profits_and_solutions (self, d, final_solution, avg_list, profit_matrix, weight_vector):
        # Plain mode: pin L2-derived patterns at the top of the order
        if self.relationship_control == False:
            true_list = [a for a, x in enumerate(final_solution) if x] 
            for i in range(d):
    ...
    def augmented_data(self):
        ...
        if self.relationship_control == False:
            ...
            solution_space, profits, weights, sorting_augmented_index_list_l3 = self.sort_profits_and_solutions(self.dimension, self.solution_space, sort_list, converted_profits, weights)
        else:
            self.profits = profits
            self.weights = weights
            solution_space, profits, weights, sorting_augmented_index_list_l3 = self.solution_space_of_selected_items(self.dimension, self.solution_space, profits, weights)
        return solution_space, profits, weights, sorting_augmented_index_list_l3
